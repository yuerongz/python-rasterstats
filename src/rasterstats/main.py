...
def gen_zonal_stats(
    ...
    **kwargs):
    ...
    weight_scale: int
        Scale used when generating weights based on percent coverage of each
        ...
        (based on weight_scale value) then using a summation to aggregate
        to the raster resolution and dividing by the square of weight_scale
        to get percent coverage value for each cell. Increasing weight_scale
        will increase the accuracy of percent coverage values; three orders
        magnitude finer resolution (weight_scale=1000) is usually enough to
        get coverage estimates with <1% error in individual edge cells coverage
        estimates, though much smaller values (e.g., weight_scale=10) are often
    ...
    if weights:
        if weight_scale is None:
            ...
            try:
                if weight_scale != int(weight_scale):
                    warnings.warn('Value for `weight_scale` given ({0}) '
                                  'was converted to int ({1}) but does not '
                                  'match original value'.format(
                                    weight_scale, int(weight_scale)))

                weight_scale = int(weights_scale)

                if weight_scale <= 1:
                    warnings.warn('Value for `weight_scale` must be greater than one '
                                  '({0})'.format(weight_scale))

            except:
                raise Exception('Invalid value for `weight_scale` provided '
                                '({0})'.format(weight_scale))

        if not all_touched:
            warnings.warn("`all_touched` was not enabled but a `weighted_<stat>` was selected. "
            ...
            if len(compressed) == 0:
                ...
                if 'weighted_mean' in stats:
                    feature_stats['weighted_mean'] = float(
                            np.sum(masked * pctcover) / np.sum(~masked.mask * pctcover))
                if 'weighted_count' in stats:
                    feature_stats['weighted_count'] = float(np.sum(~masked.mask * pctcover))
